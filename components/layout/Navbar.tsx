...
const MorphingIcon = ({ type, isActive, isHovered }: MorphingIconProps) => {
  ...
  const animateHome = () => {
    ...
    gsap.to(button, {
      ...
      duration: 0.1,
      onComplete: () => {
        const hasMorph = typeof window !== "undefined" && !!(gsap.plugins as any)?.morphSVG;
        const morphKeyframes = hasMorph
          ? [
  ...
  const resetAnimation = () => {
    ...
    const defaultPaths: Record<string, string> = {
      ...
      marker: "M12 21C12 21 9.39536 18.8605 7.3637 16C6.06474 14.1711 5 12.0475 5 10C5 6.134 8.134 3 12 3C15.866 3 19 6.134 19 10C19 12.0475 17.9353 14.1711 16.6363 16C14.6046 18.8605 12 21 12 21Z",
    ...
    const hasMorph = typeof window !== "undefined" && !!(gsap.plugins as any)?.morphSVG;
    if (hasMorph) {
      gsap.set(path, { morphSVG: defaultPaths[type] });
    }

    gsap.set(button, {
      [`--tab-bar-${type}-scale`]: 1,
  ...
  if (type === "home") {
    return (
      <span ref={containerRef} className={cn("home", isActive ? "active" : "")}>
  ...
  if (type === "blog") {
    return (
      <span ref={containerRef} className={cn("blog", isActive ? "active" : "")}>
        <svg viewBox="0 0 24 24">
  ...
  if (type === "marker") {
    return (
      <span ref={containerRef} className={cn("marker", isActive ? "active" : "")}>
        <svg viewBox="0 0 24 24">
  ...
  if (type === "email") {
    return (
      <span ref={containerRef} className={cn("email", isActive ? "active" : "")}>
        <svg viewBox="0 0 24 24">
  ...
  if (type === "linkedin") {
    return (
      <span ref={containerRef} className={cn("linkedin", isActive ? "active" : "")}>
        <svg viewBox="0 0 24 24">
  ...
  if (type === "x") {
    return (
      <span ref={containerRef} className={cn("x", isActive ? "active" : "")}>
        <svg viewBox="0 0 24 24">
  ...
  if (type === "github") {
    return (
      <span ref={containerRef} className={cn("github", isActive ? "active" : "")}>
        <svg viewBox="0 0 24 24">
                  ...
                  <div className="relative h-4 flex items-center justify-center overflow-hidden w-full">
                    <AnimatePresence mode="popLayout" custom={direction}>
                      <motion.span
                        ...
                        className="text-[13px] font-medium tracking-wide whitespace-nowrap text-current"
                      >
